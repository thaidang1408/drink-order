...
class StoreController {
  ...
  getQR = asyncHandler(async (req, res) => {
    const table = req.validatedQuery?.table ?? req.query.table;
    const qr = await this.service.getQRBySlug(req.params.slug, table);
    sendSuccess(res, qr);
  });

  downloadQR = asyncHandler(async (req, res) => {
    const table = (req.validatedQuery?.table ?? req.query.table)?.trim();
  ...
  getAdminQR = asyncHandler(async (req, res) => {
    const table = req.validatedQuery?.table ?? req.query.table;
    const qr = await this.service.getQRByStoreId(req.params.storeId, table);
    sendSuccess(res, qr);
  });
